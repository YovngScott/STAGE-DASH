...
export async function getWebAppFactoryStatus() {
  ...
  try {
    const [organizationResponse, projectsResponse] = await Promise.all([
      fetch(`https://api.supabase.com/v1/organizations/${organizationId}`, {
    ...
    const projectCount = Array.isArray(projects)
      ? projects.filter((project: any) => project.organization_id === organizationId).length
      : 0;
    ...
    return {
      ...
      projectCount,
      projectLimit,
      capacityAvailable: projectLimit === null || projectCount < projectLimit,
      tokenExpiresAt,
    };
  } catch {
    return {
...
export async function startWebAppProvision(input: WebAppFactoryInput) {
  ...
  if (templateError || !template)
    throw new Error(templateError?.message || "Plantilla no encontrada.");
...
export async function preflightWebAppFactory(input: WebAppFactoryInput) {
  const missing = [
    ...
    "STAGE_FLY_API_TOKEN",
    "STAGE_FLY_ORG_SLUG",
    "STAGE_SUPABASE_MANAGEMENT_TOKEN",
    "STAGE_SUPABASE_ORGANIZATION_ID",
  ...
  if (!managementToken.startsWith("sbp_"))
    throw new Error("El token de administración de Supabase no es válido.");
  const [github, supabase, organization, projects, fly] = await Promise.all([
    ...
    fetch(
      `https://api.supabase.com/v1/organizations/${process.env.STAGE_SUPABASE_ORGANIZATION_ID}`,
      { headers: { authorization: `Bearer ${managementToken}` } },
    ),
    fetch("https://api.supabase.com/v1/projects", {
      headers: { authorization: `Bearer ${managementToken}` },
    }),
    run("fly", ["apps", "list", "--json"], process.cwd(), flyEnv(), 30_000).then(() => ({
      ok: true,
    })),
  ]);
  ...
  if (!organization.ok || !projects.ok)
    throw new Error("No se pudo verificar la capacidad disponible en Supabase.");
  const organizationData = await organization.json();
  const projectData = await projects.json();
  const organizationProjects = Array.isArray(projectData)
    ? projectData.filter(
        (project: any) => project.organization_id === process.env.STAGE_SUPABASE_ORGANIZATION_ID,
      )
    : [];
  if (organizationData?.plan === "free" && organizationProjects.length >= 2) {
    throw new Error(
      "Supabase Free ya tiene sus 2 proyectos activos. Stage no creará recursos huérfanos: para publicar esta réplica se necesita liberar un proyecto o activar el plan Pro.",
    );
  }
  if (!fly.ok) throw new Error("Fly.io no está disponible.");
}

async function runWebAppProvision(
  row: DeploymentRow,
  sourceRepo: string,
  input: WebAppFactoryInput,
) {
  const managementToken = process.env.STAGE_SUPABASE_MANAGEMENT_TOKEN!;
  ...
  if (!templateOwner || !templateRepo || !targetOwner)
    throw new Error("Repositorio de plantilla inválido.");
  ...
  if (!projectRef)
    throw new Error(project?.message || "Supabase no devolvió el identificador del proyecto.");
  await supabaseAdmin
    .from("web_app_deployments")
  ...
  try {
    const sqlFiles = (await readdir(path.join(sourceDir, "sql")))
      .filter((name) => name.endsWith(".sql"))
      .sort(naturalSqlOrder);
    ...
    const keys = await supabaseManagement(
      managementToken,
      `/v1/projects/${projectRef}/api-keys?reveal=true`,
    );
    const keyList = Array.isArray(keys) ? keys : [];
    const publishable =
      keyList.find((key: any) => key.type === "publishable")?.api_key ||
      keyList.find((key: any) => key.name === "anon")?.api_key;
    const secret =
      keyList.find((key: any) => key.type === "secret")?.api_key ||
      keyList.find((key: any) => key.name === "service_role")?.api_key;
    if (!publishable || !secret)
      throw new Error("Supabase no entregó las llaves publicable y secreta.");
    ...
    const authResponse = await fetch(`${projectUrl}/auth/v1/admin/users`, {
      ...
      body: JSON.stringify({
        ...
        user_metadata: {
          ...
          correo_contacto: input.adminEmail,
    ...
    if (!authResponse.ok)
      throw new Error(
        `No se pudo crear el administrador (${authResponse.status}): ${(await authResponse.text()).slice(0, 300)}`,
      );
    ...
    const profileResponse = await fetch(`${projectUrl}/rest/v1/rpc/guardar_perfil_usuario`, {
      ...
      body: JSON.stringify({
        p_user_id: userId,
        p_nombre_completo: input.companyName,
        p_rol: "administrativo_general",
        p_activo: true,
        p_login_email: loginEmail,
        p_pin: input.adminPin,
      }),
    });
    if (!profileResponse.ok) {
      ...
      throw new Error(
        `No se pudo crear el perfil administrador (${profileResponse.status}): ${(await profileResponse.text()).slice(0, 300)}`,
      );
    }
    ...
    if (!exists)
      await run(
        "fly",
        ["apps", "create", row.fly_app_name!, "--org", process.env.STAGE_FLY_ORG_SLUG!],
        sourceDir,
        flyEnv(),
      );
    const runtime = {
    ...
    await run(
      "fly",
      ["secrets", "import", "--app", row.fly_app_name!],
      sourceDir,
      flyEnv(),
      300_000,
      `${secrets.join("\n")}\n`,
    ...
    await run(
      "fly",
      ["deploy", "--app", row.fly_app_name!, "--remote-only", "--yes"],
      sourceDir,
      flyEnv(),
      900_000,
    );
    ...
    const releasesRaw = await run(
      "fly",
      ["releases", "--app", row.fly_app_name!, "--json"],
      sourceDir,
      flyEnv(),
      30_000,
    );
    ...
    await supabaseAdmin
      ...
      .update({
        ...
        current_release: completed[0]?.Version ? String(completed[0].Version) : null,
        previous_release: completed[1]?.Version ? String(completed[1].Version) : null,
        state: "live",
        progress: 100,
        phase: "Aplicación publicada y verificada",
        error: null,
...
export async function rollbackWebAppDeployment(id: string) {
  const { data: row, error } = await supabaseAdmin
    .from("web_app_deployments")
    .select("id,fly_app_name,previous_release,public_url")
    .eq("id", id)
    .single();
  if (error || !row) throw new Error(error?.message || "Despliegue no encontrado.");
  if (!row.fly_app_name || !row.previous_release)
    throw new Error("Todavía no existe una versión anterior recuperable.");
  await supabaseAdmin
    .from("web_app_deployments")
    .update({ state: "rolling_back", phase: "Restaurando versión anterior…" })
    .eq("id", id);
  try {
    await run(
      "fly",
      ["releases", "rollback", row.previous_release, "--app", row.fly_app_name, "--yes"],
      process.cwd(),
      flyEnv(),
      300_000,
    );
    ...
    await supabaseAdmin
      ...
      .update({
        state: "live",
        progress: 100,
        phase: `Rollback completado a v${row.previous_release}`,
        current_release: row.previous_release,
        error: null,
        updated_at: new Date().toISOString(),
      })
      .eq("id", id);
...
async function update(id: string, progress: number, phase: string, state = "running") {
  const { error } = await supabaseAdmin
    .from("web_app_deployments")
    .update({ progress, phase, state, updated_at: new Date().toISOString() })
    .eq("id", id);
  if (error) throw new Error(error.message);
}
...
function validateInput(input: WebAppFactoryInput) {
  if (!input.clientId || !input.templateId || !input.companyName.trim())
    throw new Error("Cliente, plantilla y nombre son obligatorios.");
  if (
    !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(input.email) ||
    !/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(input.adminEmail)
  )
    throw new Error("Los correos no son válidos.");
  if (!/^\d{4}$/.test(input.adminPin))
    throw new Error("El PIN inicial debe tener exactamente 4 dígitos.");
  if (!/^#[0-9a-f]{6}$/i.test(input.brandPrimary) || !/^#[0-9a-f]{6}$/i.test(input.brandInk))
    throw new Error("Los colores de marca no son válidos.");
}
...
function sanitizeSlug(value: string) {
  return (
    value
      ...
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 48) || `cliente-${randomUUID().slice(0, 8)}`
  );
}
...
function githubHeaders() {
  return {
    ...
    authorization: `Bearer ${process.env.STAGE_GITHUB_TOKEN}`,
    "content-type": "application/json",
...
async function supabaseManagement(
  ...
  if (!response.ok)
    throw new Error(
      body?.message || body?.error || `Supabase Management API respondió ${response.status}.`,
    );
  return body;
}
...
async function waitForSupabaseProject(token: string, ref: string) {
  for (let attempt = 0; attempt < 60; attempt += 1) {
    ...
    if (["INACTIVE", "REMOVED", "PAUSED"].includes(project?.status))
      throw new Error(`Supabase quedó en estado ${project.status}.`);
...
async function downloadRepository(repo: string) {
  ...
  const response = await fetch(`https://api.github.com/repos/${repo}/tarball/main`, {
...
function run(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      child.kill("SIGTERM");
      reject(new Error(`${binary} excedió el tiempo máximo.`));
    }, timeoutMs);
    ...
    child.on("close", (code) => {
      ...
      if (code === 0) {
        ...
        reject(new Error(output.trim().slice(-1000) || `${binary} terminó con código ${code}.`));
      }
    });
  });
}
