                      ...
                      >
                        {toggling === b.botId ? (
                          ...
                          <Power
                            className={cn(
                              "h-3.5 w-3.5",
                              b.status === "active" ? "text-zinc-400" : "text-emerald-400",
                            )}
                          />
                        )}
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 bg-zinc-950/60 px-6 py-4">
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-400">
                Elige un bot y administra sus tareas y estado en vivo.
              </p>
            </div>
            <select
            ...
            >
              {bots.map((bot) => (
                <option key={bot.slug} value={bot.slug} className="bg-zinc-900 text-zinc-200">
                  {bot.clientName} · {bot.name}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="p-6">
                ...
                <div className="flex flex-wrap items-center gap-2.5">
                  ...
                  <Badge
                    ...
                    className={cn(
                      ...
                      selectedBot.runtime?.mode === "shadow" &&
                    ...
                    onClick={() =>
                      void runAction("runtime", {
                        payload: {
                          ...
                          autoSendPercentage:
                            mode === "limited"
                              ? Math.max(10, selectedBot.runtime?.autoSendPercentage ?? 10)
                              : mode === "live"
                                ? 100
                                : 0,
                        },
            ...
            <Tabs defaultValue="conversations" className="space-y-5">
              <TabsList className="grid h-11 w-full grid-cols-2 rounded-xl border border-white/10 bg-black/40 p-1 md:grid-cols-4 backdrop-blur-md">
                <TabsTrigger
                  value="conversations"
                  className="rounded-lg text-xs font-medium text-zinc-400 data-[state=active]:bg-white/10 data-[state=active]:text-white transition-all"
                >
                  Conversaciones
                </TabsTrigger>
                <TabsTrigger
              ...
              <TabsContent value="conversations" className="mt-0 grid gap-5 lg:grid-cols-2">
                {selectedBot.kind === "assistant" && (
                  <div className="rounded-xl border border-white/10 bg-black/30 p-5 backdrop-blur-md lg:col-span-2">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div>
                        <p className="flex items-center gap-2 text-sm font-semibold text-zinc-200">
                          <Mail className="h-4 w-4 text-primary" /> Seguimientos de correo
                        </p>
                        <p className="mt-1 text-xs text-zinc-400">
                          Compromisos que el asistente debe continuar. La respuesta siempre vuelve
                          al destinatario y al hilo original.
                        </p>
                      </div>
                      <Badge
                        variant="outline"
                        className="border-white/10 bg-white/5 text-zinc-300 text-xs font-normal"
                      >
                        {selectedBot.emailFollowups.length} pendientes
                      </Badge>
                    </div>
                    <div className="mt-4 max-h-80 space-y-3 overflow-auto pr-1">
                        ...
                        >
                          <div className="flex flex-wrap items-start justify-between gap-2">
                            <div className="min-w-0">
                              ...
                              <p className="mt-1 truncate text-xs text-zinc-400 font-mono">
                                {item.recipient} · {item.subject}
                              </p>
                            </div>
                            <Badge
                              variant="secondary"
                              className="border border-white/10 bg-white/5 text-zinc-300 text-[11px]"
                            >
                              {item.task_type === "calendar"
                                ? "Agenda"
                          ...
                          {item.draft_reply && (
                            <p className="mt-2 rounded-lg bg-black/40 border border-white/5 p-3 text-xs text-zinc-300 font-mono">
                              Borrador: {item.draft_reply}
                            </p>
                          )}
                          <textarea
                          ...
                          <div className="mt-2.5 flex flex-wrap gap-2">
                            <Button
                              ...
                              disabled={
                                (followupReplies[item.id] ?? "").trim().length < 25 ||
                                actionLoading === "replyFollowup"
                              }
                              onClick={() =>
                                void runAction("replyFollowup", {
                                  id: item.id,
                                  payload: { response: followupReplies[item.id] },
                                })
                              }
                            >
                            ...
                            <Button
                              size="sm"
                              variant="outline"
                              className="h-8 rounded-lg border-white/10 bg-white/5 text-zinc-300 hover:bg-white/10 text-xs"
                              disabled={actionLoading === "resolveFollowup"}
                              onClick={() =>
                                void runAction("resolveFollowup", {
                                  id: item.id,
                                  payload: {
                                    resolution: "Resuelto manualmente desde Owner Console",
                                  },
                                })
                              }
                            >
                      ...
                      {!selectedBot.emailFollowups.length && (
                        <p className="rounded-xl border border-dashed border-white/10 p-6 text-center text-xs text-zinc-400">
                          No hay compromisos de correo pendientes.
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="rounded-xl border border-white/10 bg-black/30 p-5 backdrop-blur-md">
                  <p className="flex items-center gap-2 text-sm font-semibold text-zinc-200">
                    <UserRound className="h-4 w-4 text-primary" /> Intervención humana
                  </p>
                  <p className="mt-1 text-xs text-zinc-400">
                    Toma una conversación para que el bot deje de responder temporalmente.
                  </p>
                  <div className="mt-4 max-h-56 space-y-2 overflow-auto">
                    {selectedBot.handoffs.map((item) => (
                      <div
                        key={`${item.channel}:${item.conversation_id}`}
                        className="flex items-center justify-between gap-2 rounded-lg border border-white/5 bg-zinc-950/60 p-2.5 text-xs"
                      >
                        <span className="truncate text-zinc-300 font-mono">
                          {item.channel} · {item.conversation_id}
                        </span>
                        <Button
                          size="sm"
                          variant="ghost"
                    ...
                    {!selectedBot.handoffs.length && (
                      <p className="rounded-xl border border-dashed border-white/10 p-6 text-center text-xs text-zinc-400">
                        Ninguna conversación está bajo control humano.
                      </p>
                    )}
                  </div>
                </div>
                <div className="rounded-xl border border-white/10 bg-black/30 p-5 backdrop-blur-md">
                  ...
                  <p className="mt-1 text-xs text-zinc-400">
                    Selecciona “Tomar” solamente cuando necesites intervenir.
                  </p>
                  <div className="mt-4 max-h-56 space-y-2 overflow-auto">
                    {selectedBot.conversations
                      .filter(
                        (conversation, index, all) =>
                          all.findIndex(
                            (item) =>
                              item.channel === conversation.channel && item.id === conversation.id,
                          ) === index,
                      )
                      .filter(
                        (conversation) =>
                          !selectedBot.handoffs.some(
                            (handoff) =>
                              handoff.channel === conversation.channel &&
                      ...
                      .map((conversation) => (
                        <div
                          key={`${conversation.channel}:${conversation.id}`}
                          className="flex items-center justify-between gap-2 rounded-lg border border-white/5 bg-zinc-950/60 p-2.5 text-xs"
                        >
                          <span className="min-w-0 truncate text-zinc-300 font-mono">
                            {conversation.channel} · {conversation.contact}
                            {conversation.subject ? ` · ${conversation.subject}` : ""}
                          </span>
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-7 text-xs text-zinc-300 hover:text-white shrink-0"
                            onClick={() =>
                              void runAction("take", {
                                channel: conversation.channel,
                                conversationId: conversation.id,
                                payload: { reason: "Tomada desde Owner Console" },
                              })
                            }
                          >
                            Tomar
                ...
                <div className="rounded-xl border border-white/10 bg-black/30 p-5 backdrop-blur-md lg:col-span-2">
                  <p className="text-sm font-semibold text-zinc-200">
                    Respuestas en sombra{" "}
                    <span className="text-xs font-normal text-zinc-400">
                      ({selectedBot.shadows.filter((item) => !item.reviewed).length} pendientes)
                    </span>
                  </p>
                  <div className="mt-3 grid max-h-48 gap-2.5 overflow-auto md:grid-cols-2">
                    {selectedBot.shadows
                      ...
                      .map((item) => (
                        <div
                          key={item.id}
                          className="rounded-lg border border-white/5 bg-zinc-950/60 p-3 text-xs"
                        >
                          ...
                          <Button
                            ...
                            onClick={() =>
                              void runAction("reviewShadow", { id: item.id, payload: {} })
                            }
                          >
                            Marcar revisada
                          </Button>
                    ...
                    {!selectedBot.shadows.some((item) => !item.reviewed) && (
              ...
              <TabsContent value="tests" className="mt-0 grid gap-5 lg:grid-cols-[1fr_1fr]">
                <div className="rounded-xl border border-white/10 bg-black/30 p-5 backdrop-blur-md">
                  <p className="flex items-center gap-2 text-sm font-semibold text-zinc-200">
                    <FlaskConical className="h-4 w-4 text-primary" /> Ejecutar prueba real
                  </p>
                  <p className="mt-1 text-xs text-zinc-400">
                    Stage envía un código y confirma que el canal puede recibirlo y responderlo.
                  </p>
                  <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                    <div className="space-y-2">
                      <input
                        className="h-10 w-full rounded-xl border border-white/10 bg-black/50 px-3 text-xs text-zinc-100 placeholder:text-zinc-500 outline-none backdrop-blur-sm focus:border-white/20"
                        placeholder="Correo de prueba"
                        value={emailTestDestination}
                        onChange={(e) => setEmailTestDestination(e.target.value)}
                      />
                      <Button
                        size="sm"
                        className="w-full h-9 rounded-xl border-white/10 bg-white/5 text-zinc-300 hover:bg-white/10 text-xs"
                        variant="outline"
                        disabled={!emailTestDestination || actionLoading === "channelTest"}
                        onClick={() =>
                          void runAction("channelTest", {
                            channel: "email",
                            payload: { destination: emailTestDestination },
                          })
                        }
                      >
                        Probar Gmail
                    ...
                    <div className="space-y-2">
                      <input
                        className="h-10 w-full rounded-xl border border-white/10 bg-black/50 px-3 text-xs text-zinc-100 placeholder:text-zinc-500 outline-none backdrop-blur-sm focus:border-white/20"
                        placeholder="WhatsApp +1809…"
                        value={whatsappTestDestination}
                        onChange={(e) => setWhatsappTestDestination(e.target.value)}
                      />
                      <Button
                        size="sm"
                        className="w-full h-9 rounded-xl border-white/10 bg-white/5 text-zinc-300 hover:bg-white/10 text-xs"
                        variant="outline"
                        disabled={!whatsappTestDestination || actionLoading === "channelTest"}
                        onClick={() =>
                          void runAction("channelTest", {
                            channel: "whatsapp",
                            payload: { destination: whatsappTestDestination },
                          })
                        }
                      >
                        Probar WhatsApp
                ...
                <div className="rounded-xl border border-white/10 bg-black/30 p-5 backdrop-blur-md">
                  ...
                  <div className="mt-4 max-h-48 space-y-2 overflow-auto text-xs">
                    {selectedBot.channelTests.slice(0, 8).map((run) => (
                      <div
                        key={run.id}
                        className="flex items-center justify-between rounded-lg border border-white/5 bg-zinc-950/60 p-2.5"
                      >
                        <span className="text-zinc-300 font-mono">
                          {run.channel} · {run.destination}
                        </span>
                        <Badge
                          variant={
                            run.status === "passed"
                    ...
                    {!selectedBot.channelTests.length && (
                      <p className="rounded-xl border border-dashed border-white/10 p-6 text-center text-zinc-400">
                        Todavía no hay pruebas de canal.
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="rounded-xl border border-white/10 bg-black/30 p-5 backdrop-blur-md">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                    <div className="rounded-xl border border-white/5 bg-zinc-950/60 p-3.5">
                      <p className="text-[11px] uppercase tracking-wider text-zinc-400">Mensajes</p>
                      <p className="mt-1 font-semibold text-white text-base">
                        {selectedBot.runtime?.monthlyMessages?.toLocaleString() ?? 0}
                      </p>
                    </div>
                    <div className="rounded-xl border border-white/5 bg-zinc-950/60 p-3.5">
                      <p className="text-[11px] uppercase tracking-wider text-zinc-400">Correos</p>
                      <p className="mt-1 font-semibold text-white text-base">
                        {selectedBot.runtime?.monthlyEmails?.toLocaleString() ?? 0}
                      </p>
                    </div>
                    <div className="rounded-xl border border-white/5 bg-zinc-950/60 p-3.5">
                      <p className="text-[11px] uppercase tracking-wider text-zinc-400">Tokens</p>
                      <p className="mt-1 font-semibold text-white text-base">
                        {selectedBot.runtime?.monthlyTokens?.toLocaleString() ?? 0}
                      </p>
                    </div>
                    <div className="rounded-xl border border-white/5 bg-zinc-950/60 p-3.5">
                      ...
                      <p className="mt-1 font-semibold text-white text-base">
                        ${selectedBot.runtime?.monthlyCostUsd ?? 0}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <TabsContent value="recovery" className="mt-0">
                <div className="rounded-xl border border-white/10 bg-black/30 p-5 backdrop-blur-md">
                  ...
                  <p className="mt-1 text-xs text-zinc-400">
                    Estas acciones no cambian el modo de respuesta del bot.
                  </p>
                  <div className="mt-4 flex flex-wrap gap-3">
                    <Button
                      variant="outline"
                      className="h-9 rounded-xl border-white/10 bg-white/5 text-zinc-300 hover:bg-white/10 text-xs"
                      disabled={actionLoading === "recoveryDrill"}
                      onClick={() => void runAction("recoveryDrill")}
                    >
                      <DatabaseBackup className="mr-2 h-4 w-4" /> Ejecutar simulacro
                    </Button>
                    <Button
                      variant="outline"
                      className="h-9 rounded-xl border-white/10 bg-white/5 text-zinc-300 hover:bg-white/10 text-xs"
        ...
        {!failures.length ? (
          ...
          <div className="divide-y divide-white/5">
            {failures.map(({ bot, failure }) => (
              <div
                key={failure.id}
                className="flex flex-wrap items-center gap-4 px-6 py-4 transition-colors hover:bg-white/[0.02]"
              >
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-white">
                    {bot.clientName} ·{" "}
                    <span className="font-mono text-zinc-300 text-xs">{failure.operation}</span>
                  </p>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    className="h-8 rounded-lg text-xs text-zinc-400 hover:text-white"
                    onClick={() => void resolveFailure(bot.slug, failure.id)}
                  >
                    Marcar resuelto
                  </Button>
                </div>
              ...
              return (
                ...
                >
                  ...
                  {activa ? (
                    <Badge
                      variant="destructive"
                      className="shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-medium border-rose-500/30 bg-rose-500/20 text-rose-300"
                    >
                      Activa
                    </Badge>
                  ) : (
                    <Badge
                      variant="secondary"
                      className="shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-medium border-white/10 bg-white/5 text-zinc-400"
                    >
                      Resuelta
                    </Badge>
                  )}
        ...
        <AlertDialogContent className="border-border/60 bg-zinc-950/90 backdrop-blur-xl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-zinc-300">
              ...
              dedicada en Fly.io liberando las máquinas, volúmenes e IPs reservadas, y se eliminará
              la configuración del bot en GitHub y la base de datos de Supabase.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel
