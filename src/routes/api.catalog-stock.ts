    ...
    handlers: {
      PATCH: async ({ request }) => {
        ...
        if (
          !body?.tenantId ||
          !body.serviceId ||
          (body.stock === undefined && body.precio === undefined)
        )
          return Response.json(
            { error: "tenantId, serviceId y stock o precio son obligatorios." },
            { status: 400 },
          );
        if (!/^[0-9a-f-]{36}$/i.test(body.tenantId) || !/^[0-9a-f-]{36}$/i.test(body.serviceId))
          return Response.json({ error: "Identificadores inválidos." }, { status: 400 });
        const patch: { stock?: number; precio?: number; actualizado_en: string } = {
        ...
        const { data, error } = await supabaseAdmin
          ...
          .select("id,tenant_id,nombre,precio,moneda,stock,actualizado_en")
          .maybeSingle();
        if (error) return Response.json({ error: error.message }, { status: 400 });
        if (!data)
          return Response.json(
            { error: "Servicio inexistente o fuera del tenant." },
            { status: 404 },
          );
