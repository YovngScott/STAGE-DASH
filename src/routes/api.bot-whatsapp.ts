      ...
      POST: async ({ request }) => {
        ...
        if (!statusUrl)
          return Response.json(
            { error: "Este bot no tiene una URL de estado configurada." },
            { status: 400 },
          );

        const secret = process.env.STAGE_PLATFORM_ADMIN_SECRET || storedSecret;
        if (!secret)
          return Response.json({ error: "Falta el secreto de plataforma local." }, { status: 500 });

        const whatsappUrl = statusUrl.replace(
          /\/config\/bot-activo\/?(?:\?.*)?$/,
          "/whatsapp/status",
        );
        ...
        if (!response.ok) {
          return Response.json(
            {
              error: payload?.error ?? `El bot respondió ${response.status}.`,
