    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          const { bot_status_url, dashboard_url } = await assertClientOwnsTenant(
            clientId,
            tenantSlug,
          );
          ...
          if (!firstMembership) {
            return Response.json(
              {
                error:
                  "Este cliente no tiene ningún usuario de dashboard configurado. Crea uno en la ficha de clientes primero.",
              },
              { status: 400 },
            );
          }
          ...
          try {
            ...
            throw new Error(
              `Error en el intercambio de tokens de sesión: ${verifyErr instanceof Error ? verifyErr.message : "Desconocido"}`,
            );
          }
...
function getMessagingAdmin(): MessagingAdmin {
  ...
  if (!key)
    throw new Error("Falta STAGE_MESSAGING_SUPABASE_SERVICE_ROLE_KEY para generar el enlace.");
